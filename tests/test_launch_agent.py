...
class TestGetExecutablePath:
    @mock.patch("claude_usage.launch_agent.shutil.which")
    def test_pip_install_via_which(self, mock_which):
        mock_which.return_value = "/usr/local/bin/claude-usage-bar"
        with mock.patch("claude_usage.launch_agent.os.path.realpath", side_effect=lambda p: p):
    ...
    def test_relative_path_from_which_is_resolved(self, mock_which):
        ...
        with mock.patch(
            "claude_usage.launch_agent.os.path.realpath",
    ...
    def test_sibling_of_sys_executable(self, _mock_which, _mock_valid):
    ...
    def test_app_bundle_detection(self, _mock_which, _mock_valid):
        ...
        with mock.patch("claude_usage.launch_agent.__file__", fake_path):
            result = _get_executable_path()
    ...
    @mock.patch("claude_usage.launch_agent._is_valid_executable", return_value=False)
    @mock.patch("claude_usage.launch_agent.shutil.which", return_value=None)
...
class TestInstallLaunchAgent:
    @mock.patch("claude_usage.launch_agent._get_executable_path", return_value="/usr/local/bin/claude-usage-bar")
    @mock.patch("claude_usage.launch_agent.os.makedirs")
    ...
    @mock.patch("claude_usage.launch_agent._get_executable_path", return_value="/usr/local/bin/claude-usage-bar")
    @mock.patch("claude_usage.launch_agent.os.makedirs", side_effect=OSError("permission denied"))
    ...
    @mock.patch("claude_usage.launch_agent._get_executable_path", return_value=None)
    @mock.patch("claude_usage.launch_agent.os.makedirs")
...
class TestUninstallLaunchAgent:
    @mock.patch("claude_usage.launch_agent.os.path.isfile", return_value=True)
    @mock.patch("claude_usage.launch_agent.os.remove")
    ...
    @mock.patch("claude_usage.launch_agent.os.path.isfile", return_value=False)
    def test_noop_when_absent(self, _mock_isfile):
        result = uninstall_launch_agent()
        assert result is True  # idempotent

    @mock.patch("claude_usage.launch_agent.os.path.isfile", return_value=True)
    @mock.patch("claude_usage.launch_agent.os.remove", side_effect=OSError("permission denied"))
